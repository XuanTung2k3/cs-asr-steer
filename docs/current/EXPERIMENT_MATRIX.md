...
| **DG-08 ⏳ READY FOR AUDIT** | Locked Whisper core eval, seeds [13,42,73], F0–F4 finalists. Greedy + beam-5 on locked D-test (6,257 utts/15 dialogues), 2000-rep dialogue-block bootstrap. **M\* significantly beats frozen Whisper on PIER/MER/utility in both regimes (CIs exclude zero); COMPETITIVE with SALSA/LoRA (best ASR quality+retention+inference parity; SALSA higher raw correction; LoRA MER-catastrophic).** Beam robustness PERSISTS. Outside-harm N/A on D-test (no candidate/POI alignments). Lock `1fb2c3`; report `results/dg08/DG08_RESULTS_SUMMARY.md`. mig-only; no post-test tuning | train `loc-train ∪ util-train`; select `D-dev-select`; **`D-test`** final (no D-dev-confirm selection) | FD |
