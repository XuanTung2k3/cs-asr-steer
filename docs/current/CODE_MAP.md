...
| **DG-07 learned variants** (LB1/LB2/A1/A2/A3) | `src/csasr/steering/dg07_variants.py`, runner `experiments/dg07_baselines_ablations.py`, frozen config `configs/dg07_baselines_ablations.yaml`, spec `docs/current/DG07_BASELINES_ABLATIONS_SPEC.md`, launcher `sbatch/cs_asr_dg07_baselines_ablations.sh` | **canonical pre-run implementation (DG-07A, no outcomes).** LB1 uses the DG-02 exact site and one global learned vector; LB2 uses matched-budget Q/V LoRA in decoder L24; A1/A2/A3 use a gate-only trunk with frozen local/conditioning/fixed `[0.5,0.5]` directions. All reuse D1's `C_E` + matrix KL objective, L24, β, seed, budget, greedy D-dev-select selection. A5 basis refinement is deferred because no `λ_A` was predeclared. GPU launch is `mig`-only and is not submitted in DG-07A |
| **Exact-site scientific free-decoding runner** (DG-05+) | `experiments/dg05_adaptive_controller.py`; `experiments/dg06_damage_aware.py` | **DG-05B and DG-06 implemented/frozen.** Both use `sites.DecoderPostCrossAttnInterventionHook`, greedy/temperature-0/beam-1 decoding, canonical `result_v1`, and provenance manifests on D-dev-select; no confirmation/test path is present |
