...
| **DG-03 causal screen** (C0–C4, free decoding) | `experiments/dg03_causal_screen.py` (launcher `sbatch/cs_asr_dg03_screen.sh`) | **canonical (DG-03, FROZEN).** Reuses frozen DG-02 hook + `job_a_frozen.oracle_steps_for` (oracle diagnostic gate); every condition (incl. C0) is a complete validated `result_v1` (metrics+gains+POI transitions+3 retention populations+reserved gate_coverage) with per-utterance texts; per-condition edit-count/total-energy recorded, C4 count-matched to C1. Selected **L24** (jobs 50471/50472; `results/dg03/screen/`). Candidate-level `n_corrupted_outside` deferred to the DG-04 candidate frontier |
