...
**Separate inference-time steering proposal — current state (2026-09-25): P2 COMPLETE — `P2_VALID_CONFIGURATION_SELECTED` → `READY_FOR_P3_LOCKED_EVALUATION` (frozen: L16, α=2.0, φ_id, g=E·R_B, +d; `results/inference_cf/p2_frozen_configuration.json`; `docs/inference_cf/P2_REPORT.md`, `P2_AUDIT.md` PASS). `OPEN DECISION` before P3 (not started): development effect is only ΔPIER +2/2268 vs the matched α=0 baseline (CI95 [0, 0.0027]); B0_AUTO is 8.2 PIER points better; P2-B controls (energy-matched g=1 +5, −d +11, g=R_B +10) do not support detector selectivity or the +d direction.** Earlier: Pre-P2 COMPLETE (`CACHED_EQUIVALENCE: PASS`, job 54802; `PASS_TO_P2_DEVELOPMENT`). P2-A attempt 1 (jobs 54803/54804) = `P2_BLOCKED_INVALID_EXPERIMENT` (baseline execution mismatch; preserved, unused); spec v1.1 (`04d2383`) matched α=0 `cached_decode` baseline + divergence attribution; P2-A r1 jobs 54821/54822, P2-B 54824, P2-C 54825.
