...
| **Canonical steering-basis builder** (`v_raw`, `v_cond`, `v_local`, `V^0`; MC §4) | `src/csasr/directions/steering_basis.py` — `build_basis`, `raw_language_contrast`, `language_conditioning`, `conditioning_residualized_local`, `write_basis_artifact` (`steering_basis_v1`); construction runner `experiments/dg03_build_basis.py` | **implemented; DG-03 freeze audit blocked.** v6 aggregate-then-residualize order; finite/unit/orthogonal validation and deterministic tensor hashing pass. Basis tensors are built at L16/L24 (job 50452), but artifact provenance is incomplete (`dataset_fingerprint`/`construction_config_hash` null). Not a relabel of legacy `v_nat` |
| **DG-03 causal screen** (C0–C4, free decoding) | `experiments/dg03_causal_screen.py` (launcher `sbatch/cs_asr_dg03_screen.sh`) | **implemented; freeze audit blocked.** Reuses frozen DG-02 hook + `job_a_frozen.oracle_steps_for` (oracle diagnostic gate) + `metrics_v1` `paired_corpus_report`; selected L24 numerically, but screen artifacts are not complete `result_v1` records, omit retention/outside-harm fields, and do not record C4 edit-count/total-energy matching |
