...
| **Adaptive controller** `f_θ(LN(r_t)) → (g_t, π_t)` (MC §6) | `src/csasr/steering/controller.py`, `src/csasr/steering/dg05_training.py`, `experiments/dg05_adaptive_controller.py`, `configs/dg05_adaptive_controller.yaml` | **DG-05A canonical implementation / READY FOR TRAINING.** L24 and both DG-03 basis hashes are guarded; the 32-wide bottleneck emits sigmoid gate + softmax rank-2 mixture and uses the exact-site hook's dynamic action path. Correction-only `C_E` helpers are implemented; no GPU training or free-decoding checkpoint result exists yet. Legacy F5/T1 gates are **not** this controller |
...
| **Exact-site scientific free-decoding runner** (DG-05+) | **DG-05B selection/evaluation runner absent** | **gap.** DG-03/DG-04 canonical runners are implemented above; DG-05A supplies only the correction-only training path. Later runners must remain on `sites.DecoderPostCrossAttnInterventionHook` (FROZEN) and write a full provenance manifest; legacy runners steer post-FFN |
...
| Contract correction + retention losses | `src/csasr/steering/dg05_training.py` (correction-only CE) | **DG-05A correction-only component implemented; DG-06 gap remains for explicit KL-to-baseline retention on `ℛ_E`/`ℛ_M` and optional anchor.** Job B's `F.cross_entropy` at `experiments/job_b_training.py:1016` is all-token CE (legacy) |
