...
| **Exact-site scientific free-decoding runner** (DG-04+) | **Absent for post-DG-03 stages** | **gap.** DG-03's canonical screen is implemented above; later runners must remain on `sites.DecoderPostCrossAttnInterventionHook` (FROZEN) and write a full provenance manifest; legacy runners steer post-FFN |
