...
**Status:** P0/P0-R1 completed; original geometry/candidate-support gate is blocked. The unsteered P0-R2 repairability gate is specified in `docs/inference_cf/P0_R2_REPAIRABILITY_SPEC.md` (v1.1), implemented, and independently audited `PASS_TO_R2_RUN` (`docs/inference_cf/P0_R2_REPAIRABILITY_AUDIT.md`): **READY_FOR_R2_RUN**. No R2 GPU outcome or P1 intervention exists. Before P1, `g_cf` is described only as a repair-need ranking score / counterfactually language-responsive matrix-script conflict, never proven repairability.  
...
| **P0-R2 — revised repair-need feasibility** | **READY_FOR_R2_RUN (audited), UNRUN.** The independent design audit, formula reconciliation, and repairability spec are complete. Commit and prepare the clean-state manifest, then evaluate the exact contract on the dense, existing 300-utterance D-dev-select panel. No steering intervention or alpha/layer tuning. | Freeze or reject the revised gate; compare old and counterfactual gates. Separate verdicts for local support, both conflicts, alignment, and combined repair-need discrimination. |
| **P1 — working causal method** | After R2 passes, implement the smallest frozen-model path: per-prefix `hE-hM`, the frozen R2 gate, norm-preserving edit at the existing engineering anchor, isolated branch/cache semantics, and standard reference-free decoding. | One integrated GPU acceptance covering alpha=0 identity, real eligible edit, cache/mask/beam behavior, direction/gate plumbing, and serialization. This is the first stage that tests an intervention. |
