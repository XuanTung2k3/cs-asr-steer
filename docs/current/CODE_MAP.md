...
| **Canonical steering-basis builder** (`v_raw`, `v_cond`, `v_local`, `V^0`; MC §4) | `src/csasr/directions/steering_basis.py` — `build_basis`, `raw_language_contrast`, `language_conditioning`, `conditioning_residualized_local`, `write_basis_artifact` (`steering_basis_v1`), `diagnostic_dose`, `CONTRACT_LAYERS`; construction runner `experiments/dg03_build_basis.py` | **canonical (DG-03).** v6 aggregate-then-residualize order; validated (finite/unit/orthogonal) + deterministic hashing; tests `tests/test_steering_basis.py` (10). Built & frozen at L16/L24 (`results/dg03/basis/`, job 50452). Not a relabel of legacy `v_nat` |
| **DG-03 causal screen** (C0–C4, free decoding) | `experiments/dg03_causal_screen.py` (launcher `sbatch/cs_asr_dg03_screen.sh`) | **DG-03 runner.** Reuses frozen DG-02 hook + `job_a_frozen.oracle_steps_for` (oracle diagnostic gate) + canonical `paired_corpus_report`. Selected **L24** (jobs 50453/50454; `results/dg03/screen/`) |
