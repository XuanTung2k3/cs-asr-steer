...
| **DG-06 ⏳ READY FOR AUDIT** | Damage-aware training (MC §8): D0 (reuse DG-05) / D1 (+matrix KL retention) / D2 (+embedded KL retention), `λ_M=λ_E=1.0`, β/layer/basis fixed, D1==D2 reconstructed DG-05 init (jobs 50558/50559 on `mig`). Both flip MER/matrix-CER from harm→gain and cut outside harm ~73%; **`SELECT D1`** (net 85 > D2 net 68). Outcome **MATRIX-RETENTION SUCCESS; EMBEDDED-RETENTION NOT SUPPORTED**. Gate penalty + basis refinement deferred. Impl status only; scientific verdict pending audit | train `loc-train ∪ util-train`; select `D-dev-select` | TF optimize / FD select |
