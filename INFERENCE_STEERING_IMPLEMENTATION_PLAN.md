...
**Next concrete task:** `OPEN DECISION` (human): whether to run P3 locked evaluation of the frozen P2 configuration (`results/inference_cf/p2_frozen_configuration.json`) given the negligible development effect (ΔPIER +2/2268) and negative P2-B mechanism controls (`docs/inference_cf/P2_REPORT.md`), or to revise the method first. If P3 proceeds, it must first freeze the P3 corpus-by-system matrix, providers and damage tolerances (checklist items below) before any test/transfer outcome; P3 is not started. Pre-P2 and P2 are complete (see status line).
