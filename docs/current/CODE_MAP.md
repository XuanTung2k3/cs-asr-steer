...
| **Exact-site scientific free-decoding runner** (DG-05+) | `experiments/dg05_adaptive_controller.py` | **DG-05B implemented for the fixed development run.** It uses `sites.DecoderPostCrossAttnInterventionHook`, greedy/temperature-0/beam-1 decoding, canonical `result_v1`, and a provenance manifest on D-dev-select; no confirmation/test path is present |
...
| Contract correction + retention losses | `src/csasr/steering/dg05_training.py` (correction-only CE) | **DG-05B correction-only component exercised; DG-06 gap remains for explicit KL-to-baseline retention on `ℛ_E`/`ℛ_M` and optional anchor.** Job B's `F.cross_entropy` at `experiments/job_b_training.py:1016` is all-token CE (legacy) |
